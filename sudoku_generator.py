...
class SudokuGenerator:
    ...
    def fill_box(self, row_start, col_start):
        boxfill_list = []
        while len(boxfill_list) < 9:
            i = random.randint(1, 9)
            if i in boxfill_list:
                continue
            else:
                boxfill_list.append(i)

        index = 0
        for i in range(3):
            for j in range(3):
                self.board[row_start+i][col_start+j] = boxfill_list[index]
                index += 1
    ...
    def remove_cells(self):
        removedCell_coords = set()
        while len(removedCell_coords) < self.removed_cells:
            coord_tuple  = (random.randint(0, self.row_length - 1), random.randint(0, self.row_length - 1))
            removedCell_coords.add(coord_tuple)

        for coord in removedCell_coords:
            row, col = coord
            self.board[row][col] = 0
